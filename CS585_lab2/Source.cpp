...
int main()
{
	...
	*/
	...
	myMotionHistory.push_back(fMH1);
	for (int i = 1; i < historyLength-1; i++) { //I don't know why it loops until i < historyLength-1 instead of i < historyLength; the array ends up being too large for the latter...
		Mat fMH2 = fMH1.clone();
		myMotionHistory.push_back(fMH2);
	}
...
void myMotionEnergy(vector<Mat> mh, Mat& dst, int length) {
	for (int i = 0; i < dst.rows; i++){
		for (int j = 0; j < dst.cols; j++){
			for (int k = 0; k < length-1; k++) { //I don't know why it loops until k < length-1 instead of k < length; the array ends up being too large for the latter...
